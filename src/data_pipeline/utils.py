...
if max_calls_per_minute < 1:
    raise ValueError("MAX_LLM_CALLS_PER_MINUTE must be a positive integer")
llm_limiter = SimpleRateLimiter(max_calls_per_minute, 60)
...
async def process_html(filepath, df, original_df, client, prompt, type):
    """
...
def create_agenda_html(agenda_df):
    """
    Create an HTML webpage for previewing agenda documents.

    Args:
        agenda_df (pandas.DataFrame): A DataFrame containing columns:
            ["body", "meeting_date", "filepath", "title"].
    """
